...
const decline = () => {
    ...
    if (
        dateSelect.value &&
        timestamp.value &&
        selectedValue.value === "Нет свободного места. Предложить свою дату."
    ) {
        data["suggested_date"] = timestamp.value;
    }

    axios
        .patch(route("order.decline", data))
        .then((response) => {
        ...
        <main>
            <div class="top">
                ...
                <div class="w-1/2 mx-auto">
                    <p
                        class="text-violet-100 text-base font-normal font-['Open Sans'] leading-tight text-center my-8"
                    >
                        Выберите причину отказа от проекта
                    </p>
                    <n-select
                    ...
                    <button
                        ...
                        class="w-full py-2 my-2 btn_gradient-purple animation hover:bg-purple-800 rounded-3xl text-violet-100 text-lg font-bold font-['Open Sans'] leading-normal"
                    >
                        Подтвердить
                    </button>
                </div>
