...
<template>
    <div class="order_card">
        <div class="p-4 order_card-container">
            <div class="grid" :class="{ '!grid-cols-2' : order.status === 'pending' }">
                <div>
                        ...
                        <div class="flex justify-between border-t-2 border-violet-100 border-opacity-40 py-4">
                           <div class="flex">
                               <div class="flex flex-col items-end">
                                   ...
                                   <div class="text-violet-100 text-base font-bold font-['Open Sans'] leading-tight flex items-center gap-x-2"><base-icon size="30" :path="mdiCheck"/>{{$t('messages.' + order.status)}}</div>
                               </div>
                           </div>
                        </div>
                    </div>
                ...
                <div v-if="order.status !== 'pending'" class="flex flex-col justify-center">
                    <template v-if="order.status === 'check'">
                        <div class="text-center text-violet-100 text-sm font-bold font-['Open Sans'] leading-tight">
                            Вы отправили пост на проверку
                        </div>
                    </template>
                    <template v-else-if="order.status === 'accepted'">
                        <div class="text-center text-violet-100 text-sm font-bold font-['Open Sans'] leading-tight">Разместите пост</div>
                        <div class="text-center py-2 text-violet-100 text-sm font-normal font-['Open Sans'] leading-tight">С {{ formattedPubDate }} {{ formattedPubTime }} <br/> По {{formattedPubDateEnd}} {{ formattedPubTimeEnd}}</div>
                    ...
                    <template v-else-if="order.status === 'declined'">
                        <div class="text-center text-violet-100 text-sm font-bold font-['Open Sans'] leading-tight">{{order.decline_reason}}</div>
                    </template>
                </div>
            </div>
        </div>
