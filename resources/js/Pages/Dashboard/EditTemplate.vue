...
const isEmptyEditor = computed(
    () =>
        !!(
            ...
            content.value.ops &&
            content.value.ops.length === 1 &&
            content.value.ops[0].insert === "\n"
        )
);
...
const patchPattern = async function () {
    ...
    images.value.forEach((image) =>
        formData.append("media[]", image.file || image.url)
    );
...
<template>
    ...
    <TemplateLayout>
        ...
        <template #editor>
            ...
            />
            <QuillEditor
                @ready="(methods, quill) => onEditorReady(methods, quill)"
        ...
        <template #file-upload>
            <div class="flex flex-col gap-y-8">
                ...
                <div>
                    <label
                        class="cursor-pointer px-6 py-2 bg-blue-950 transition hover:bg-transparent rounded-full shadow-inner border border-white border-opacity-10 text-violet-100 text-sm font-normal font-['Open Sans'] leading-tight custom-file-upload"
                    >
                        <input
                            type="file"
                            multiple
                    ...
                    <transition-group name="delete">
                        ...
                        >
                            <template
                                v-if="
                                    image.url &&
                                    ['.mp4', '.ogg', '.webm'].some((ext) =>
                                        image.url.includes(ext)
                                    )
                                "
                            >
                                <img
                                    @click.prevent="openVideoPlayer(image)"
                                    v-if="
                                        ['.mp4', '.ogg', '.webm'].some((ext) =>
                                            image.url.includes(ext)
                                        )
                ...
                >
                    ...
                    >
                        <template v-if="image.file">
                            ...
                            />
                            <video
                                v-else-if="image.file.type.includes('video/')"
                                :src="image.url"
                                loop
                                muted
                                class="w-full h-full"
