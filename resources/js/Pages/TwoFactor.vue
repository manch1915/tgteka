...
const verify = () => {
    loading.start();
    axios
        .post(route("two-factor.verify"), form)
        .then((response) => {
            loading.finish();
...
<template>
    ...
    <MainLayout>
        ...
        >
            ...
            <div>
                <n-input
                    v-model:value="form.two_factor_code"
                    type="text"
                ...
                <button
                    ...
                    class="w-full px-6 py-2 my-2 btn_gradient-purple transition hover:bg-purple-800 rounded-full text-violet-100 text-lg font-bold font-['Open Sans'] leading-normal"
                >
                    Подтвердить
                </button>
            </div>
